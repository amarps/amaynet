...
TCP::TCP() : _file_descriptor(0), _is_use_ssl(false) {}
...
TCP::TCP(const TCP &other)
    : _port(other._port), _file_descriptor(other._file_descriptor),
      _is_use_ssl(other._is_use_ssl), ssl_ctx(other.ssl_ctx),
...
std::vector<char> TCP::Recv(size_t buf_size)
{
  if (IsUseSSL()) {
...
std::vector<char> TCP::RecvSSL(size_t buf_size)
{
...
std::vector<char> TCP::RecvTCP(size_t buf_size) const
{
  char recv_buf[buf_size];
...
SSL *TCP::InitSSL(const std::string &hostname)
{
...
int TCP::Close() const { return close(_file_descriptor); }
