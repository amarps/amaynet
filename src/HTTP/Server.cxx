...
HTTPRequest HTTPServer::GetRequest()
{
  ...
  std::string str_req_methods[num_method_element]{"GET /", "POST /"};

  for (int mthd_i = 0; mthd_i < num_method_element; mthd_i++) {
    if (msg.compare(0, str_req_methods[mthd_i].length(),
...
void HTTPServer::ServeResource(std::string &path, int chunk_size)
{
  ...
  client->Send(std::string("Content-Type: ").append(ct).append("\r\n"));
...
HTTPServer::HTTPServer(const std::string &port) : TCPListener(port) {}
