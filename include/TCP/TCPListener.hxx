...
public:
   ...
   */
  bool IsConnectionEnd() { return m_connection.iter() == m_connection.end(); }

  TCP *CurrentConnection() { return m_connection.data(); }

  /**
   * @return true if CurrentConnection is ready to receive data
   */
  bool IsConnectionReady() { return m_connection.Ready(); }

  void DropConnection() { m_connection.Drop(); }

  void NextConnection() { m_connection.Next(); }

  // reset currentconnection to the first element of connection
  void ConnectionBegin() { m_connection.MoveToBegin(); }
